...
def plot_one_box_kpt(x, im, color=None, label="Normal", line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    if label == "Normal":
            ...
            cv2.putText(im, label, (c1[0], c1[1] - 2), 0, tl / 6, [0, 255, 0], thickness=tf//2, lineType=cv2.LINE_AA)
    else:
            cv2.rectangle(im, c1, c2, (0,0,255), -1, cv2.LINE_AA)  # filled
            cv2.putText(im, label, (c1[0], c1[1] - 2), 0, tl / 6, [0, 0, 255], thickness=tf//2, lineType=cv2.LINE_AA)
    if kpt_label:
...
def plot_one_box(x, img, color=None, label="Normal", line_thickness=1):
